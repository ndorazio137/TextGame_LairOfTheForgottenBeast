...
public class StaticWorldMapGenerator implements WorldMapGenerator {
  ...
  public WorldMap generateWorldMap(int sizeX, int sizeY, int sizeZ) {
    ...
    rooms[0][0][0].addProp(propFactory.create("Item", "iron sword", "a small shortsword",
        "a small sword forged from iron. good for cutting things."));
    rooms[0][0][0].addProp(
        propFactory.create("Item", "healing potion", "a potion", "a potion which restores health"));
    Item weapon1 = new Item("iron sword", "a small shortsword",
    ...
    Item torch = (Item) propFactory.create("Item", "torch", "a torch",
    ...
    rooms[6][3][0].addProp(propFactory.create("Item", "bronze sword", "a small shortsword",
        "a small sword forged from bronze. good for cutting things."));
    rooms[6][3][0].addProp(
        propFactory.create("Item", "bronze dagger", "a large knife", "a large knife for combat."));
    rooms[6][3][0].addProp(propFactory.create("Item", "suit of armor", "body armor",
    ...
    rooms[4][4][0] = new RoomDynamic(5, "Dining Hall",
        "This is the dining hall. Tables and chairs fill the available space.");
    rooms[4][4][0].addProp(
    ...
    rooms[4][6][0].addProp(propFactory.create("Decoration", "book", "a book.",
    ...
    rooms[5][4][0] = new RoomDynamic(9, "Kitchen",
        "The kitchen is a room with an iron pot over a cooking fire.");
    ...
    rooms[7][6][0].addProp(propFactory.create("Item", "obsidian knife", "an obsidian knife",
        "a ceremonial knife made from volcanic glass."));
    ...
    rooms[7][4][0]
        .addProp(propFactory.create("Decoration", "bed", "a bed.", "a bed for sleeping."));
    ...
    rooms[7][7][0].addProp(propFactory.create("Decoration", "giant vault", "a giant stone vault.",
        ...
        /*
         * Answer: If Komur were telling the truth, then there would be two knights.  
         * We know there is only one knight, so he is not telling the truth.  
         * Komur could be either a spy or a knave. If Harulon was the knight,  
         * then her statement would be a lie. Harulon cannot be the knight.  
         * If neither Komur nor Harulon can be the knight, then Jerroth must be the knight. Komur, 
